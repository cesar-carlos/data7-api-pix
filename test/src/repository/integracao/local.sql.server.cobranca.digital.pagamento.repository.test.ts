...
import LocalSqlServerCobrancaDigitalPagamentoRepository from '../../../../src/repository/integracao/local.sql.server.cobranca.digital.pagamento.repository';

describe('CRUD (local.sql.server.cobranca.digital.pagamento.repository)', () => {
  const repository = new LocalSqlServerCobrancaDigitalPagamentoRepository();
  const newEntity = new CobrancaDigitalPagamentoDto({
    sysId: '3833579.11.27740308000120.20220830180236-OB.21 999',
    sequencia: 1,
    status: 'A',
  ...
  it('deve ler registro gravado', async () => {
    const params = [
      { key: 'SysId', value: '3833579.11.27740308000120.20220830180236-OB.21 999' },
