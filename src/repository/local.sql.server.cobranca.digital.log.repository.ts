...
import CobrancaDigitalLogDto from '../dto/cobranca.digital.log';
  ...
  implements LocalBaseRepositoryContract<CobrancaDigitalLogDto>
{
  ...
  private async actonEntity(entity: CobrancaDigitalLogDto, sqlCommand: string): Promise<void> {
    try {
      ...
      await transaction
        .request()
        .input('ID', VarChar(500), entity.id)
        .input('Message', VarChar(2000), entity.message)
        .input('Details', VarChar(2000), entity.details)
        .query(sqlCommand);
